...
def load_and_prepare_dataset(dataset_path: str, tokenizer):
    ...
    def format_chat_messages(example):
        """Format prompt-response pairs for pretrained model (RL-compatible format)."""
        ...
        # For pretrained models, check if chat template exists and is not None
        # Pretrained models don't have chat templates, so use manual formatting
        # Check both that the attribute exists AND that it's not None
        chat_template = getattr(tokenizer, 'chat_template', None)
        has_chat_template = chat_template is not None and chat_template != ""
        
        if has_chat_template:
            # Try to use chat template if available (for Instruct models)
            try:
                messages = [
            ...
            # Manual format for pretrained models (RL-compatible)
            # This format works well with AutoModelForCausalLMWithValueHead
            formatted_text = f"{system_message}\n\nUser: {user_message}\n\nAssistant: {assistant_message}"
...
def train_instruction_model(config: InstructionTuningConfig):
    ...
    # Ensure pad token is set (critical for RL training compatibility)
    ...
    # For pretrained models, ensure tokenizer has necessary attributes for RL
    if not hasattr(tokenizer, 'chat_template') or tokenizer.chat_template is None:
        logger.info("Pretrained model detected - using manual formatting for RL compatibility")
    
    # Save tokenizer config to ensure RL compatibility
    ...
    # Save final model (ensure RL compatibility)
    logger.info(f"Saving final model to {config.output_dir}")
    trainer.save_model()
    
    # Save tokenizer with all necessary configs for RL training
    tokenizer.save_pretrained(config.output_dir)
    
    # Ensure model config is saved properly for RL compatibility
    model.config.save_pretrained(config.output_dir)
    
    # Verify model can be loaded (important for RL training)
    logger.info("Verifying model can be loaded for RL training compatibility...")
    try:
        from trl import AutoModelForCausalLMWithValueHead
        test_model = AutoModelForCausalLMWithValueHead.from_pretrained(
            config.output_dir,
            device_map="cpu",  # Just for verification
            torch_dtype=torch.bfloat16
        )
        logger.info("✓ Model is compatible with AutoModelForCausalLMWithValueHead (RL training)")
        del test_model
    except Exception as e:
        logger.warning(f"Model compatibility check failed: {e}")
        logger.warning("Model may still work, but verify before RL training")
    ...
    logger.info("Model is ready for RL training with AutoModelForCausalLMWithValueHead")
...
Generate a deep-reasoning physics question that tests a student's understanding of the relationship between displacement and the variables initial_velocity, acceleration, time"""
    ...
    for idx, example in enumerate(test_examples, 1):
        ...
        # Format as chat (compatible with pretrained models)
        has_chat_template = (
        ...
        if has_chat_template:
            ...
            # Manual format for pretrained models
            formatted_input = f"{system_message}\n\nUser: {test_prompt}\n\nAssistant:"
